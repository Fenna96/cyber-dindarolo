...
from .models import Catalog,Product, BALANCE_LIMIT, Category
...
class InsertCatalogForm(ModelForm):
    class Meta:
        model = Catalog
    ...
    def clean_price(self):
        ...
        if price > BALANCE_LIMIT:
            raise forms.ValidationError(("You can't sell at more than price limit!"), code='price_limit')
        return price

    def __init__(self, *args, **kwargs):
        super(InsertCatalogForm, self).__init__(*args, **kwargs)
        for visible in self.visible_fields():
            visible.field.widget.attrs['class'] = 'form-control'
            visible.field.widget.attrs['placeholder'] = visible.name.capitalize()
            visible.initial = ""