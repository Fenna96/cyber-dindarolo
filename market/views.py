...
def insert(request):
    ...
    context = {
        "balance": utils.get_balance(request),
        "catalog_form": forms.InsertCatalogItemForm(prefix="catalog"),
...
def catalog(request, error: str = None):
    ...
    user = request.user
    catalog = models.CatalogItem.objects.filter(
        ~Q(user=user)
    )  # mostra solo ciò che vendono gli altri!
    """
    ...
    for key in keys:
        ...
        new_dict["items"] = sorted(
            new_dict["items"], key=lambda item: (item.price, item.quantity)
        )
...
def add_item(request):
    if request.method == "POST":
        ...
        catalog_form = forms.InsertCatalogItemForm(request.POST, prefix="catalog")
        if product_form.is_valid() and catalog_form.is_valid():
            ...
            item = models.CatalogItem.objects.filter(
            ...
            if item:
                ...
                )  # aggiorno la quantità sommandole
                item.save()
            else:
                print(product)
...
def buy_item(request):
    ...
    if request.method == "POST":
        ...
        try:
            item = models.CatalogItem.objects.get(id=item_id)
        except:
        ...
        tracking = models.Product_pricetracker(
            seller=seller, product=product, price=price
        )
        tracking.save()
