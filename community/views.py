...
from user.models import Profile, User, DEFAULT_PIC
...
def leaderboard(request):
    ...
    context = {'balance': utils.get_balance(request), 'leaderboard': leaderboard, 'top10': top10, 'worse10':worse10}
    if request.method == 'GET':
        return HttpResponse(template.render(context,request))
    return HttpResponse(template.render(context, request), status=HTTPStatus.FOUND)
...
def community_profile(request,username):
    ...
    if request.method == 'GET':
        try:
            ...
            return HttpResponseRedirect(reverse('community:search', args=('User not found',)))
        context = {'balance': utils.get_balance(request), 'user':user,'profile': profile,'profile_balance':balance}
...
def modify(request,email_error:str=None):
    ...
    context = {'balance': utils.get_balance(request), 'profile': profile, 'form': ProfileForm,'profile_balance':balance,'email_error':email_error}
    if request.method == 'GET':
        return HttpResponse(template.render(context,request))
    return HttpResponse(template.render(context, request), status=HTTPStatus.FOUND)
...
def change_profile(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                if (yep.username != user.username):
                    ...
                    return HttpResponseRedirect(reverse('community:modify', args=(error,)))
                user.save()
                profile.save()  # salvataggio in BACKEND
                return HttpResponseRedirect(reverse('community:community_profile', args=(user.username,)))
            except:
                user.save()
                profile.save()  # salvataggio in BACKEND
                return HttpResponseRedirect(reverse('community:community_profile', args=(user.username,)))
        ...
        context = {'balance': utils.get_balance(request), 'profile': profile, 'form': form,
...
def change_pic(request):
    if request.method == 'POST':
        ...
        if profile_image:
            ...
            profile.profile_image = DEFAULT_PIC
        profile.save()  # salvataggio in BACKEND
        return HttpResponseRedirect(reverse('community:community_profile', args=(request.user.username,)))
    """
    Probabilmente errore nello scrivere, quindi vieni reindirizzato alla pagina giusta
    """
    return HttpResponseRedirect(reverse('community:modify'))
    ...
    if request.method == 'POST':
        try:
            ...
            if(username):
                try:
                    user = User.objects.get(username=username)
                    return HttpResponseRedirect(reverse('community:community_profile', args=(username,)))
                except:
