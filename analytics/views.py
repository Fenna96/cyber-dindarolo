    ...
    if request.method == "GET":
        ...
        if your_history:
            ...
            for opt in options:
                ...
                if opt != "settimana":
                    pieChart[opt] = sum_category(
                        ordered_transaction[-1]["group"], models.Category.objects.all()
                    )

            context = {
                "balance": utils.get_balance(request),
                "transactions": transactions,
                "history": your_history.order_by("-date"),
                "lineChart": lineChart,
                "pieChart": pieChart,
            }
            ...
            return render(
                ...
                {"error": error, "balance": utils.get_balance(request)},
            )
    ...
    if request.method == "GET":
        ...
        if product:
            lineChart = group_by_user(
                request,
                models.Product_pricetracker.objects.filter(product=product),
                item,
            )
            context = {
                "balance": utils.get_balance(request),
            ...
            if item:
                ...
                try:
                    ...
                    return HttpResponseRedirect(
                        reverse("analytics:price_tracker", args=(item,))
                    )
                except:
                    error = "Product not found"
                    return HttpResponseRedirect(
                        reverse("analytics:search", args=(error,))
                    )
